...
void Estimator::clearState()
{
    ...
    last_marginalization_parameter_blocks.clear();  // 边缘化相关向量清0

    f_manager.clearState();     // 特征管理清0
...
{
    if ( !first_imu )   // 如果是第一帧的话
    {
        first_imu = true;
        acc_0 = linear_acceleration;    // acc_0 = 传入的IMU测得的加速度参数
        gyr_0 = angular_velocity;       // acc_0 和 gyr_0 都是 上一时刻的加速度和角速度
    }

    if ( !pre_integrations[frame_count] )   // 如果当前帧对应的预积分为空
    {
    ...
    if ( frame_count != 0 )
    {
        ...
        // 下面三步暂存一下IMU预积分之后的时间, 加速度, 角速度
        dt_buf[frame_count].push_back(dt);
        linear_acceleration_buf[frame_count].push_back(linear_acceleration);
        angular_velocity_buf[frame_count].push_back(angular_velocity);

        int j = frame_count;    // Rs[j] = Rwb
...
bool Estimator::initialStructure()
{
    ...
    GlobalSFM sfm;
    //std::cout << "wrong wrong weong wrong wrong!!!!!!!!!!!"<< endl;
...
void Estimator::backendOptimization()
{
    ...
    problemSolve();
    // 优化后的变量处理下自由度
    double2vector();    // 更新加速度和陀螺仪bias  Bas[]和Bgs[]
