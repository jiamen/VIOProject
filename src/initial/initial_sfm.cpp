//
// Created by zlc on 2020/4/8.
//

#include "initial/initial_sfm.h"

// 第6讲PPT Pose0和Pose1是World系到Camera系的变换矩阵, point0和point1是各自相机坐标系下归一化平面上的点
                        ...
                        Vector2d &point0, Vector2d &point1, Vector3d &point_3d)
{
    ...
    design_matrix.row(2) = point0[2] * Pose1.row(2) - Pose1.row(0);
...
{                                                                     // 当前帧号
    ...
    {
        if ( sfm_f[j].state != true )       // 已被三角化的特征点直接跳过
            continue;

        Vector2d point2d;
        for ( int k = 0; k < (int)sfm_f[j].observation.size(); k ++ )
        {   // vector<pair<int, Vector2d>> observation 是 所有观测到该特征点的图像帧ID和图像坐标
    ...
    if ( int(pts_2_vector.size()) < 15 )    // 所有特征点在当前帧上 被观测到的数量 < 15,, 证明移动过快
    {
