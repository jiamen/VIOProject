...
#ifndef _JMVIO_SPLINE_H_
#define _JMVIO_SPLINE_H_

#pragma once     // 保证头文件只被编译一次
            // 都是为了编译效率，但是有些编译器(比如bcc)不支持#pragma once
            // 这种方式，所以经常会组合使用，既确保效率，又保证移植性。
            // (不认识#pragma once的编译器会直接忽略)
...
public:
    ...
    // Constructor takes the boundary conditions as arguments, this sets
    // the first derivative (gradient) at the lower and upper end points
